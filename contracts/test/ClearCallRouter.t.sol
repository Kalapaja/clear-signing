...
contract ClearCallRouterTest is Test {
    ...
    function test_Gas_DirectCall() public {
        ...
        bytes memory directCallData = abi.encodeWithSelector(
        ...
        (bool success, ) = address(router).call(directCallData);
        uint256 gasUsed = startGas - gasleft();
        assertTrue(success, "Direct call failed");

        console2.log("Direct Call Gas Used:", gasUsed);
    }

    function test_Gas_ClearCall() public {
        address[] memory path = new address[](2);
        path[0] = address(0x1);
        path[1] = address(0x2);

        bytes memory clearCallData = getClearCallData(100, 90, path, address(this), block.timestamp);

        uint256 startGas = gasleft();
        (bool success, ) = address(router).call(clearCallData);
        uint256 gasUsed = startGas - gasleft();
        assertTrue(success, "Clear call failed");

        console2.log("Clear Call Gas Used:", gasUsed);
    }

    function test_Gas_GetDisplayHash() public {
        uint256 startGas = gasleft();
        bytes32 hash = SwapExactTokensForTokensDisplayHash.getSwapExactTokensForTokenDisplayHash(router);
        uint256 gasUsed = startGas - gasleft();

        assertTrue(hash != bytes32(0));
        console2.log("Get Display Hash Gas Used:", gasUsed);
    }

    function _calculateCalldataGas(bytes memory data) internal pure returns (uint256) {
        uint256 gasCost = 0;
