...
- **Context Variables**:
    ...
    - **`$args`**: The primary storage for resolving parameter values. Initialized with function arguments from
...
- **Explicit Scoping**: The display format uses a recursive structure where some fields MAY contain nested `fields`. 
  Control flow types like `map` and `array` create new scopes by processing their own `fields` array with a new `$args`
...
**Control Flow Types**
These types create a new `$args` scope from their `params` and recursively process their `fields` array. The `$msg`
context remains unchanged:

- **`map`**: Acts as a **data scope transformation**. It supports mapping values into a new scope's `$args` using:
    ...
    - **ABI Decoding**: By providing **`abi`** (signature) and **`value`** (bytes) parameters, `map` decodes the bytes
      and maps the resulting fields into `$args`.
...
1. **Context Initialization**:
    ...
    - The **$args context** MUST be initialized. For the top-level call, `$args` maps to the function arguments from
      `$msg.data`.

2. **Field Processing**:
    - Field processing accepts the `$msg` and `$args` contexts and recursively processes a `fields` array to return a
    ...
    - **Scoped Processing (`map`, `array`, `switch`)**: When a field is of type `map`, `array`, or `switch`:
        - A new `$args` context is created based on the field's `params`.
...
3. **Variable Resolution Rules**:
    ...
    - **$args**: The primary storage for resolving parameter values.
        - **Top-Level**: Initialized by decoding the function arguments from calldata (`$msg.data`) according to the
          `abi`. (e.g., `transfer(address to, uint256 amount)` -> `$args.to`, `$args.amount`).
        - **Scoped Contexts**: When entering a `map`, `array`, or `switch` scope, a **new** `$args` object is
    ...
    - **Resolution Failure**: If a referenced path (e.g., `$args.missingParam`) does not exist in the context,
        ...
        keccak256(abi.encodePacked(
            keccak256(abi.encode(
                ...
                keccak256(abi.encodePacked(
                    ...
                    keccak256(abi.encode(ENTRY_TH, keccak256(bytes("amount")), keccak256(bytes("$args.amount"))))
                )),
